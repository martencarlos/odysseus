...
# ── Module-level email read cache — populated when setup_email_routes() runs
# These are the cannonical cache objects; the local closures inside
# setup_email_routes() alias them so external callers (chat_routes,
# background pollers) can also access the cache without importing
# closure-scoped functions that don't exist at module level.
_EMAIL_READ_CACHE = {}
_EMAIL_READ_TTL = 30 * 60.0  # 30 minutes


def _read_cache_key(account_id, folder, uid, owner=""):
    """Module-level read-cache key builder — mirror of the email-routes closure."""
    return (account_id or "", folder, str(uid), owner)


def _read_cache_get(key):
    """Module-level read-cache lookup — mirror of the email-routes closure."""
    import time as _t

    v = _EMAIL_READ_CACHE.get(key)
    if not v:
        return None
    if v[0] < _t.monotonic():
        _EMAIL_READ_CACHE.pop(key, None)
        return None
    return v[1]


def _read_cache_put(key, value):
    """Module-level read-cache store — mirror of the email-routes closure."""
    import time as _t

    _EMAIL_READ_CACHE[key] = (_t.monotonic() + _EMAIL_READ_TTL, value)
    if len(_EMAIL_READ_CACHE) > 256:
        for k in list(_EMAIL_READ_CACHE.keys())[:-128]:
            _EMAIL_READ_CACHE.pop(k, None)
...
def setup_email_routes():
    ...
    _FOLDER_TTL = 5 * 60.0
    # Alias the module-level read cache so all closure callers and external
    # importers share the same dict.
    _READ_CACHE = _EMAIL_READ_CACHE
    _READ_TTL = _EMAIL_READ_TTL
    ...
    def _read_cache_key(account_id, folder, uid, owner=""):
        # Delegate to module-level so external callers share the same impl.
        from routes.email_routes import _read_cache_key as _mod_key

        return _mod_key(account_id, folder, uid, owner=owner)
    ...
    def _read_cache_get(key):
        # Delegate to module-level function so external callers (chat_routes,
        # background pollers) share the same cache implementation.
        from routes.email_routes import _read_cache_get as _mod_get

        return _mod_get(key)

    def _read_cache_put(key, value):
        from routes.email_routes import _read_cache_put as _mod_put
    ...
    async def translate_email(data: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            marker = re.search(
                r"<<<TRANSLATION>>>\s*(.*?)\s*<<<END>>>", content, re.S | re.I
            )
            if marker:
                content = marker.group(1).strip()
            else:
                content = re.sub(
                    r"^\s*<<<TRANSLATION>>>\s*", "", content, flags=re.I
                ).strip()
